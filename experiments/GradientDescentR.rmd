...
wts<-descent[[1]]
losses<-descent[[2]]
q<-ggplot()+geom_point(aes(x=x,y=y))+geom_line(aes(x=x,y=wts[1,1]*x+wts[2,1]))+ggtitle("Data with fitted line")
p<-ggplot()+geom_line(aes(x=seq(length(losses)),y=losses))+xlab("Iterations")+ylab("Loss")+ggtitle("Gradient Descent Loss")
