...
To create a `zip` file you use the `zip` command.  The `-r` option
means to descend into directories as well.
...
**WARNING:** `cut` (and `sort`) are not sophisticated about quoted fields that contain commas,
unlike, say, pandas or the tidyverse.  So you may not always get what you are expecting if you have quoted
